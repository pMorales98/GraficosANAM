...
datos_gasolina = 
  datos_gasolina %>% 
  mutate(etiqueta_leyenda = case_when(str_detect(etiqueta_leyenda, "(?i)\\(5\\)") ~ "Regular",
                                      str_detect(etiqueta_leyenda, "(?i)\\(6\\)") ~ "Premium",
                                      T ~ str_to_sentence(etiqueta_leyenda)))

# Escalar datos -------


factor_escala <- max(datos_gasolina_gg$valor_corregido, na.rm = TRUE) / 
  max(datos_ieps_gg$promedio_estimulo_magna, na.rm = TRUE)
  ...
  grafica =
    ...
    geom_ribbon(
      ...
      aes(
        x = mes,
        ymin = 0,
        ymax = promedio_estimulo_magna * factor_escala,
        fill = "Estímulo IEPS"
      ),
      alpha = 0.2
    ) +
...
ggsave(
  "../../graficas/gasolinas.png",
  grafica,
  width = 15.66,   # <-- 6850 px originales menos 10 píxeles
  height = 8.47,  # <-- 2598 px originales menos 10 píxeles
